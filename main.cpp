...
int convertCh(int chValue) {
  return map(chValue, TX12_MIN, TX12_MAX, CRSF_CHANNEL_VALUE_1000, CRSF_CHANNEL_VALUE_2000);
}
...
void loop() {
  ...
  if (autoTransmition) {
    ...
    canvas.setCursor(rightCellStart, topRowStart + spacer);
    ...
    if (pitch > 1001 && prevPitch == 0) {
      prevPitch = pitch;
      prevPitchStartMillis = millis();
    } else if (prevPitch && pitch == 1001) {
